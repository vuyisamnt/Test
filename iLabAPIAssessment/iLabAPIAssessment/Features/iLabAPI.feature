...
Scenario: Add new pet with auto generated name and status available and retrieve pet using the category id
	Given I set the required endpoint
	When I set up the body and submit POST request with name '<name>', category Id '<categoryId>' and status '<status>'
	And I confirm the json has the name '<name>' with category id '<categoryId>' 
	Then I should see new pet on the list
	When I submit GET request in get pets by category id
	Then I should see pet is returned from request '<name>'
	
Examples:	
	| name       | categoryId | status    |
	| VuyisaTest | 50         | available |
